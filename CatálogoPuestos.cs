...
namespace MAD
{
    public partial class CatálogoPuestos : Cabecera
    {
        ...
        public CatálogoPuestos()
        {
            InitializeComponent();
        }

        private void CátalogoPuestos_Load(object sender, EventArgs e)
        {
            ConfigDataGridView();
        ...
        private void FillDataGridView(int i, int f)
        {
            ...
            foreach (var row in ListaPuestos)//Departamentos
            {
                SumaTotal = 0;
                dataGridView1.Rows.Add("----------------------");
                dataGridView1.Rows.Add(row.Puesto);
                List<Empleado> ListaEmpleados = EmpleadoDAO.ObtEmpPorIdPuesto(row.IdPuesto);
