...
namespace MAD
{
    public partial class CatálogoDepartamentos : Cabecera
    {
        ...
        public CatálogoDepartamentos()
        {
            InitializeComponent();
        }
        private void CatálogoDepartamentos_Load(object sender, EventArgs e)
        {
            ConfigDataGridView();            
        }
        private void FillDataGridView(int i, int f)
        {
            ...
            foreach (var row in ListaDepartamentos)//Departamentos
            {              
                SumaTotal = 0;
                dataGridView1.Rows.Add("----------------------");
                dataGridView1.Rows.Add(row.Departamento);
                List<Empleado> ListaEmpleados = EmpleadoDAO.ObtEmpPorIdDep(row.IdDep);
                foreach (var row2 in ListaEmpleados)//Empleados
                {
                    dataGridView1.Rows.Add(row2.Nombre);
                    List<Periodo> ListaPeriodos = PeriodoDAO.ObtPeridosEnUnRango(i,f);
                    foreach (var row3 in ListaPeriodos)//Periodos
                    {
                        ...
                        if (row3.IdPeriodo >= LastDate && row3.IdPeriodo <= f)
                        {
                            CalcularTotales(row2, row3.IdPeriodo);                           
                        }
                    }
        ...
        private void CalcularTotales(Empleado empleado, int periodo)
        {
            try
            {
                ...
                dataGridView1.Rows.Add(periodo.ToString("C2"), PercepcionesGenerales.ToString("C2"), OtrasPercepciones.ToString("C2"), TotalPercepciones.ToString("C2"), "",
